...
public class NetconfHelper {
    ...
    public void openSession(final CredentialHolder credentialHolder) throws JSchException, InterruptedException, IOException {
        final JSch jsch = new JSch();
        session = jsch.getSession(credentialHolder.getUserName(), credentialHolder.getIPAddress(), credentialHolder.getPort());
        ...
        session.connect();
        Thread.sleep(100l);
        channel = session.openChannel(credentialHolder.getChannelType());
        ((ChannelSubsystem) channel).setSubsystem(credentialHolder.getChannelName());
        ((ChannelSubsystem) channel).setPty(true);
        outputStream = channel.getOutputStream();
        inputStream = channel.getInputStream();
        channel.connect();
        System.err.println("Session is live : " + session.isConnected() + ", " + "connection is live : " + channel.isConnected());
    ...
    public void closeSession() throws IOException, JSchException {
        ...
        channel.disconnect();
        session.disconnect();
        System.out.println("Session is live : " + session.isConnected() + ", " + "connection is live : " + channel.isConnected());
    }
    ...
    private String readResponse() throws IOException, JSchException {
        ...
        while ((line != null && !line.contains(RPC_REPLY)) && !line.equalsIgnoreCase(NETCONF_MESSAGE_END)) {
            responseText += line;
            responseText += "\n";
    ...
    public String runQuery(final String query) throws IOException, JSchException {
        write(query);
        final String response = readResponse();
    ...
    private void write(final String query) throws IOException {
